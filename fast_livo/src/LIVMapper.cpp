...
void LIVMapper::readParameters(rclcpp::Node::SharedPtr &node)
{
  ...
  p_pre->blind_sqr = p_pre->blind * p_pre->blind;  // 关键修复：计算blind_sqr
...
void LIVMapper::handleVIO() 
{
  ...
  // VIO XYZRGB cloud publishing removed — LIO stage now publishes
  // full 360° PointXYZINormal cloud for navigation on the same topic.
...
{    
  ...
  *pcl_w_wait_pub = *laserCloudWorld;
  ...
  // Always publish full LIO world cloud (PointXYZINormal) for navigation.
  // In LIVO mode, publish_frame_world (VIO stage) only publishes camera-FOV
  // XYZRGB points, which is insufficient for 360° navigation.
  {
    sensor_msgs::msg::PointCloud2 lio_cloud_msg;
    pcl::toROSMsg(*pcl_w_wait_pub, lio_cloud_msg);
    lio_cloud_msg.header.stamp = this->node->get_clock()->now();
    lio_cloud_msg.header.frame_id = "camera_init";
    pubLaserCloudFullRes->publish(lio_cloud_msg);
  }
...
void LIVMapper::img_cbk(const sensor_msgs::msg::Image::ConstSharedPtr &msg_in)
{
  ...
  if (last_timestamp_lidar > 0) {
    double cam_lidar_dt = msg_header_time - last_timestamp_lidar;
    RCLCPP_INFO_THROTTLE(this->node->get_logger(), *this->node->get_clock(), 2000,
        "[TimeSync] cam-lidar dt=%.4f s  (cam=%.6f lidar=%.6f imu=%.6f)",
        cam_lidar_dt, msg_header_time, last_timestamp_lidar, last_timestamp_imu);
  }
...
bool LIVMapper::sync_packages(LidarMeasureGroup &meas)
{
  ...
  // Image timeout fallback: if in LIVO mode but no image arrives for too long,
  // degrade to LIO-only processing to keep odometry and point cloud alive.
  if (img_buffer.empty() && img_en)
  {
    if (slam_mode_ == LIVO && !lid_raw_data_buffer.empty() && last_timestamp_img > 0)
    {
      double lid_newest_time = lid_header_time_buffer.back()
          + lid_raw_data_buffer.back()->points.back().curvature / double(1000);
      if (lid_newest_time - last_timestamp_img > img_timeout_sec_)
      {
        if (!livo_degraded_)
        {
          livo_degraded_ = true;
          RCLCPP_WARN(this->node->get_logger(),
              "Image timeout (%.1fs), degrading LIVO -> LIO-only",
              lid_newest_time - last_timestamp_img);
        }
        // Reset lio_vio_flg so LIVO state machine doesn't wait for VIO
        meas.lio_vio_flg = WAIT;
        // Fall through to ONLY_LIO path below
      }
      else
      {
        return false;
  ...
  else if (img_en && livo_degraded_)
  {
    // Image came back — restore LIVO mode
    livo_degraded_ = false;
    meas.lio_vio_flg = WAIT;
    RCLCPP_INFO(this->node->get_logger(), "Image resumed, restoring LIVO mode");
  }
  if (imu_buffer.empty() && imu_en) return false;

  // When degraded, process as ONLY_LIO regardless of slam_mode_
  SLAM_MODE effective_mode = livo_degraded_ ? ONLY_LIO : slam_mode_;
