...
void fromStr(const std::string &str, std::string &file_name, Pose &pose)
{
    ...
    if (tokens.size() != 8)
    {
        throw std::runtime_error(
            "Invalid pose line: expected 8 tokens, got " + std::to_string(tokens.size()) +
            " in \"" + str + "\"");
    }
    file_name = tokens[0];
...
public:
    ...
    void loadParameters()
    {
        ...
        {
            throw std::runtime_error("config_path parameter is empty; set it to a valid YAML file");
        }
        if (!std::filesystem::exists(config_path))
        {
            throw std::runtime_error("config_path does not exist: " + config_path);
        }
        ...
        {
            throw std::runtime_error("Failed to parse YAML file " + config_path + ": " + e.what());
        }
