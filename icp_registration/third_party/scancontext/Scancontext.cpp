...
void SCManager::rebuildTree()
{
    ...
    polarcontext_invkeys_to_search_.assign(
        polarcontext_invkeys_mat_.begin(), polarcontext_invkeys_mat_.end() );

    polarcontext_tree_.reset();
    polarcontext_tree_ = std::make_unique<InvKeyTree>(
        PC_NUM_RING /* dim */, polarcontext_invkeys_to_search_, 10 /* max leaf */ );

    // Also rebuild the batch tree for detectLoopClosureIDBetweenSession / detectBestMatchForScan
    polarcontext_tree_batch_.reset();
...
} // SCManager::rebuildTree


std::pair<int, float> SCManager::detectBestMatchForScan( const pcl::PointCloud<SCPointType> & _scan_down )
{
    int loop_id { -1 };
    ...
    Eigen::MatrixXd curr_desc = makeScancontext(_scan_down);
    Eigen::MatrixXd ringkey = makeRingkeyFromScancontext( curr_desc );
    std::vector<float> curr_key = eig2stdvec( ringkey );

    // Rebuild batch tree if needed
    ...
    int num_candidates = std::min(NUM_CANDIDATES_FROM_TREE,
                                  static_cast<int>(polarcontext_invkeys_mat_.size()));
    ...
    last_match_dist = min_dist;

    float yaw_diff_rad = deg2rad(nn_align * PC_UNIT_SECTORANGLE);
    return std::make_pair(loop_id, yaw_diff_rad);

} // SCManager::detectBestMatchForScan


std::pair<int, float> SCManager::detectBestMatchAmongCandidates(
    const pcl::PointCloud<SCPointType> & _scan_down,
    const std::vector<int>& candidate_indices)
{
    int loop_id { -1 };

    if( candidate_indices.empty() || polarcontexts_.empty() )
    {
        return std::make_pair(loop_id, 0.0f);
    }
    ...
    // Linear search over only the provided candidate indices
    for ( int idx : candidate_indices )
    {
        if( idx < 0 || idx >= static_cast<int>(polarcontexts_.size()) )
            continue;

        MatrixXd polarcontext_candidate = polarcontexts_[ idx ];
        std::pair<double, int> sc_dist_result = distanceBtnScanContext( curr_desc, polarcontext_candidate );
...
} // SCManager::detectBestMatchAmongCandidates
