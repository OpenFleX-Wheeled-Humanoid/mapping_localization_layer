...
struct Config
{
    ...
    double sc_dist_thresh = 0.2;
    double sc_max_radius = 80.0;
    int sc_num_exclude_recent = 30;
    int sc_num_candidates = 3;

    // NDT parameters
    double ndt_resolution = 1.0;
    double ndt_step_size = 0.1;
    int ndt_max_iterations = 30;
    double ndt_epsilon = 0.01;
...
public:
    ...
    // Save ScanContext database to disk
    bool saveScanContextDatabase(const std::string &dir_path);
...
private:
    ...
    pcl::NormalDistributionsTransform<PointType, PointType> m_ndt;
