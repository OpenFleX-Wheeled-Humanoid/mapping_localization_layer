...
void LidarProcessor::process(SyncPackage &package)
{
    ...
    // Resize buffers to match actual point count
    int cur_size = m_cloud_down_lidar->size();
    m_cloud_down_world->resize(cur_size);
    m_norm_vec->resize(cur_size);
    m_effect_cloud_lidar->resize(cur_size);
    m_effect_norm_vec->resize(cur_size);
    m_nearest_points.resize(cur_size);
    m_point_selected_flag.assign(cur_size, false);
    trimCloudMap();
    m_kf->update();
    incrCloudMap();
}
